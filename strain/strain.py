...
zSnapshotMinG = tDelayMinG = tLbMinG = zMergeMinG = rMinG = McMinG = +inf
zSnapshotMaxG = tDelayMaxG = tLbMaxG = zMergeMaxG = rMaxG = McMaxG = -inf
TotMergers = 0
...
for SS in range( SSmin, SSmax+1 ):
    ...
    # --- Local min/max ---
    zSnapshotMin = tDelayMin = tLbMin = zMergeMin = rMin = McMin = +inf
    zSnapshotMax = tDelayMax = tLbMax = zMergeMax = rMax = McMax = -inf
    ...
    McMin        = min( McMin,              Mc.min() )
    McMax        = max( McMax,              Mc.max() )
    ...
    with open( LogFile, 'a' ) as fL:
        fL.write( LogFileFMT.format( \
          ...
          tLbMax, zMergeMin, zMergeMax, rMin, rMax, McMin, McMax ) )
...
with open( LogFile, 'a' ) as fL:
    ...
    fL.write( LogFileFMT.format( \
      ...
      tLbMaxG, zMergeMinG, zMergeMaxG, rMinG, rMaxG, McMinG, McMaxG ) )
